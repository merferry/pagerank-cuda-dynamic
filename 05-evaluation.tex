...
% In Gunrock, we begin with a frontier that contains all vertices in the graph and end when all
% vertices have converged. Each iteration contains one advance operator to compute the PageRank
% value on the frontier of vertices, and one filter operator to remove the vertices whose PageRanks
% have already converged. We accumulate PageRank values with AtomicAdd operations.
